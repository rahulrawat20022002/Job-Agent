...
## Intern, Thesis or Working Student, Evaluating and Improving LLM based SE Solutions in SAP HANA

Master's student in Data Science and Analytics at SRH Heidelberg, based in Mannheim, with hands on experience building and evaluating LLM powered systems end to end. I built an LLM as Judge evaluation harness that scores answers on 5 dimensions in JSON mode at temperature 0 with a hard failure on a missing judge model, and a fairness by design credit scoring system backed by unit tests at 100 percent branch coverage and a full regulatory write up. Comfortable designing evaluation metrics and benchmarks that expose real quality gaps in non deterministic AI systems, in Python.
